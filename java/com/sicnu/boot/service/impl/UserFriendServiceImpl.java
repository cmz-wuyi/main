...
public class UserFriendServiceImpl implements UserFriendService {
    ...
    public ServerResponse<String> examineFriend(FriendExamine friendExamine) {
        ...
        if (friendExamine.getStatus() == 1 && returnCount > 0){
            ...
            //添加好友成功之后，对redis进行更新
            updateRedisUserFriend(friendExamine.getUserId());
            updateRedisUserFriend(friendExamine.getFriendId());
            log.info("同意申请成功");
     ...
     */
    private void updateRedisUserFriend(Integer userId) {
        //从数据库中获取更新
        List<UserDetail> cacheList = userFriendMapper.getFriendsByUserId(userId);
        if (!cacheList.isEmpty()){
            ...
            //清空redis
            redisUtils.deleteCacheList(REDIS_FRIEND_PREFIX + userId);
        }
    }
    ...
    public ServerResponse<String> deleteFriend(Integer friendId) {
        ...
        //删除好友成功之后，对redis进行更新
        updateRedisUserFriend(userId);
        updateRedisUserFriend(friendId);
